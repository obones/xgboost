...
class CheckpointManagerSuite extends FunSuite  with BeforeAndAfterAll {
  ...
  private lazy val (model4, model8) = {
    ...
    (XGBoost.trainWithRDD(trainingRDD, paramMap, round = 2, sc.defaultParallelism, nWorkers = 4),
      XGBoost.trainWithRDD(trainingRDD, paramMap, round = 4, sc.defaultParallelism, nWorkers = 8))
  }
