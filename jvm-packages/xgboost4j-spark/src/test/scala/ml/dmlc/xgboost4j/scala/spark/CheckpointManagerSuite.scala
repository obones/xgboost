...
class CheckpointManagerSuite extends FunSuite  with BeforeAndAfterAll {
  ...
  private lazy val (model4, model8) = {
    ...
    (XGBoost.trainWithRDD(trainingRDD, paramMap, round = 2, nWorkers = 4, sc.defaultParallelism),
      XGBoost.trainWithRDD(trainingRDD, paramMap, round = 4, nWorkers = 8, sc.defaultParallelism))
  }
